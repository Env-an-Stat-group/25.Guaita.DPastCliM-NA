function [mu_gO_trend_local,mu_mov_ESM_trend] = compute_mu_gO_trend_local_v1(tgt_ESM_mth, tgt_lon, tgt_lat, time_ESM_mth, flag_cal, name_var, metaTable)
    ...
    % Initialize output
    mu_gO_trend_local = nan(height(metaTable), length(time_ESM_mth));
    ...
    for i_time = 1:length(time_ESM_mth)
        ...
        for i_ID = 1:height(metaTable)
            ...
            mu_gO_trend_local(i_ID, i_time) = metaTable.mu_gO(i_ID) + mu_adjtrend_tmp;
        end
    end
end