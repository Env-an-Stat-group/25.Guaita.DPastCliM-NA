...
%% create downscaled maps (PCR)

disp('create downscaled maps (PCR)')
tic

save_name = fullfile(path_output, ...
    [name_var '_' name_model '_' name_experiment '_raw_PCRdownscaled' suffix '.mat']);
load(save_name)
...
for i = 1:n_stations
    [~, idx] = min((tgt_latgrid(:) - y_pcr(i)).^2 + (tgt_longrid(:) - x_pcr(i)).^2);
...
for k = 1:n_unique
    ...
    lat_filtered(k) = mean(y_pcr(idx));
    lon_filtered(k) = mean(x_pcr(idx));
...
disp(['---> had ' int2str(length(x_pcr)) ' calibration stations'])
x_pcr = metaTable.lon;
y_pcr = metaTable.lat;
disp(['---> now have ' int2str(length(x_pcr)) ' stations'])

[y_proj, x_proj] = projfwd(proj, y_pcr, x_pcr);  % first lat, then lon in degrees
...
for i_iter = 1:ceil(numel(time_ESM)/n_iter)
    ...
    %% save the dataset downscaled with PCR
    ...
    var_desc = {
        ...
        'Downscaled variable map (realizations) from PCR (lon x lat x month)',...
        'Time array in months since 0 CE (month x year, corresponds to the dimension of the maps)',...
    ...
    save_name = fullfile(path_output, ...
        [name_var '_' name_model '_' name_experiment '_PCRdownscaled' suffix '_' int2str(idx_iter(1)) '-' int2str(idx_iter(end))]);
    ...
    ncfile = fullfile(path_output, ...
        [name_var '_' name_model '_' name_experiment '_PCRdownscaled' suffix '_' int2str(idx_iter(1)) '-' int2str(idx_iter(end)) '.nc']);
    ...
    %% save the mean predicted downscaling dataset with PCR (and prediction intervals)
    
    % save realizations and mean prediction + prediction intervals separately
    ...
    var_desc = {
        ...
        'Name of the variable that is being downscaled',...
        'Name of the PMIP experiment that is being downscaled',...
        'Prediction interval (mean prediction) from PCR (lon x lat x month x boundary)',...
    ...
    save_name = fullfile(path_output, ...
        [name_var '_' name_model '_' name_experiment '_PI_PCRdownscaled' suffix '_' int2str(idx_iter(1)) '-' int2str(idx_iter(end))]);
    ...
    % Save the dataset downscaled with PCR to NetCDF
    % File name for NetCDF
    ncfile_pcr = fullfile(path_output, ...
        [name_var '_' name_model '_' name_experiment '_PI_PCRdownscaled' suffix '_' int2str(idx_iter(1)) '-' int2str(idx_iter(end)) '.nc']);
    
    % Create the NetCDF file
    ncid = netcdf.create(ncfile_pcr, 'NETCDF4'); % Create a new NetCDF file
    ...
    for i_var = 1:length(var_save)
        ...
        unit_text = var_unit{i_var};
        % skip opt_gpr
        if strcmp(var_name,'opt_gpr') || strcmp(var_name,'unit_var')
        else
            var_data = eval(var_name); % Evaluate the variable
