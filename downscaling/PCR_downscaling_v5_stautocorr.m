%PCR downscaling - code description. this uses autocorrelation in space and
%time (ARMA) to generate residuals
%
% PR GUAITA - Jan 2025
    ...
    case 'tas'
        for i_ID = 1:height(metaTable)            
            ...
            if metaTable.flag_cal(i_ID)
                ...
                residual_mat(i_ID,idx_time)=obsTable_tmp.u;
            end
        end
    case 'pr'
        for i_ID = 1:height(metaTable)            
...
%% calibrate ARMA(p,q) model on the residual matrix
...
disp(['DJF - Lambda = ' num2str(lambda) '; thresh = ' num2str(threshold_best/1000) ' km']);

%%

n_lag = 10;
pcorr = zeros(size(eps_mat,1),n_lag+1);
qcorr = zeros(size(eps_mat,1),n_lag+1);
for i_ID = 1:size(eps_mat,1)
    if metaTable.flag_cal(i_ID)
...
[u_mat] = inverse_SEM(eps_ARMA, W, lambda);
...
var_desc = {
    ...
    'Downscaled variable matrix (mean prediction) from PCR (station x time)',...
    'Residuals calculated with ARMA model and transformed back from SEM',...
...
save_name = fullfile(path_output, ...
    [name_var '_' name_model '_' name_experiment '_raw_PCRdownscaled' suffix]);
...
ncfile_pcr = fullfile(path_output, ...
    [name_var '_' name_model '_' name_experiment '_raw_PCRdownscaled' suffix '.nc']);
