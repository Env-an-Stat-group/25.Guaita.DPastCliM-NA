function plot_mean_bias_extremes_lr(dsValue_map, ESM_map, ...
    obsValue_test_map, ...
...
% Define the function to create figures for the two bias extremes (lower and upper)
    function plot_bias_for_extremes(data_map, data_map_esm, percentile_1, percentile_2, title_text_prefix, save_suffix)
        ...
        geosurfm_sub(plot_map_value_1, flag_land, ...
            lat, lon, bias_limit, ...
            [], [], [ 'LR - ' num2str(percentile_1) 'th perc.'], ...
            lim_lat, lim_lon, ...
        ...
        text(1, -0.03, sprintf(['%.2f ± %.2f ' unit_var], mean_val, std_val), ...
            'HorizontalAlignment', 'right', 'VerticalAlignment', 'top', 'Units', 'normalized', 'FontSize', 9);
        ...
        geosurfm_sub(plot_map_esm_1, flag_land, ...
            lat, lon, bias_limit, ...
            [], [], [ 'ESM - ' num2str(percentile_1) 'th perc.'], ...
            lim_lat, lim_lon, ...
            bias_step, palette_bias(n_color_bias), path_shp_file, res_fig);
        ...
        geosurfm_sub(plot_map_value_2, flag_land, ...
            lat, lon, bias_limit, ...
            [], [], [ 'LR - ' num2str(percentile_2) 'th perc.'], ...
            lim_lat, lim_lon, ...
            bias_step, palette_bias(n_color_bias), path_shp_file, res_fig);
        ...
        geosurfm_sub(plot_map_esm_2, flag_land, ...
            lat, lon, bias_limit, ...
            [], [], [ 'ESM - ' num2str(percentile_2) 'th perc.'], ...
            lim_lat, lim_lon, ...
            bias_step, palette_bias(n_color_bias), path_shp_file, res_fig);
        ...
        sgtitle([title_text_prefix ' for ' name_var ' LR (testing; mth=' i_mth_txt ')'], 'FontSize', 16, 'FontWeight', 'bold');
        ...
        set(cb, 'Position', [0.93, 0.1, 0.02, 0.8]); % [left, bottom, width, height]
        
        % Define figure save path
        name_figuresave = fullfile(path_fig, [name_var '_mean bias ' save_suffix ' ' i_mth_txt suffix]);
...
plot_bias_for_extremes(dsValue_map, ESM_map, 10, 90, 'Mean bias of 10th and 90th perc.', 'extremes_LR');
end
