function plot_eof_maps_full(eof_all, n_pc_array, tgt_size, flag_land, tgt_lat, tgt_lon, eof_limit, ...
    lim_lat, lim_lon, eof_step, palette_eof, path_shp_file, res_fig, name_var, i_mth_txt, unit_var, path_fig, suffix)
    % This function creates a set of EOF map plots (up to 16) with shared colorbar
    % Input:
    ...
    %   n_color_eof    - Number of colors in the EOF palette (for indexing)

    % Select the EOFs from the provided array
    eof_select = eof_all(:, n_pc_array);

    % Create a new figure with specified size
    f = figure('Position', [50, 50, 900, 700]);

    % Initialize a variable to collect all subplot axes
    axes_handles = gobjects(16, 1);

    % Loop through panels (up to 16 EOFs)
    for i_panel = 1:min(length(n_pc_array), 16)
        ...
        % Create subplot (4 rows, 4 columns)
        ax = subplot(4, 4, i_panel); % Assign to ax for customization
    ...
    sgtitle(['EOF for ' name_var ' (mth=' i_mth_txt ')'], 'FontSize', 16, 'FontWeight', 'bold');
    ...
    name_figuresave = fullfile(path_fig, [name_var '_eof_complete_' i_mth_txt suffix]);
