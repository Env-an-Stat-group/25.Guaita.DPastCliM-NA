...
clear y_* valid_mask *_tmp obsValue_test_map dsValue_map_lr
...
for i_season = 1:length(ind_season)
    i_mth_txt       = i_mth_txt_loop{i_season};
    flag_season     = eval(flag_loop{i_season});
    plot_rmse_timeline(rmse_timeline_PCR, rmse_timeline_LR, rmse_timeline_ESM, flag_season, time_array,...
            name_var, i_mth_txt, suffix, path_fig, res_plot,year_start_hist,year_end)
end

close all

clear rmse_timeline_*  *_lr *_mat obsTable_mat_test ESM_mat_test flag_nan_se

disp ('plotted rmse timeline')
...
clear dsValue_map 
obsValue_4d    = single(reshape(obsValue_map,length(lon),length(lat),12,[]));
clear obsValue_map 
ESM_4d    = single(reshape(tgt_ESM,length(lon),length(lat),12,[]));
clear tgt_ESM 
dsPIValue_5d     = single(reshape(PI_map,length(lon),length(lat),12,[],2));
clear PI_map
...
obsValue_4d(not(flag_land_4d)) = nan;
ESM_4d(not(flag_land_4d)) = nan;
dsPIValue_5d(not(flag_land_5d)) = nan;

clear flag_land_4d flag_land_5d obsValue_test_4d
...
regions = struct();
regions(1).name = 'A-SA';                  regions(1).lat               = [51 68]; regions(1).lon = [-170 -55];
regions(length(regions)+1).name = 'NW';  regions(length(regions)).lat = [37 51]; regions(length(regions)).lon = [-130 -102]; %North-West
regions(length(regions)+1).name = 'SW';  regions(length(regions)).lat = [23 37]; regions(length(regions)).lon = [-125 -100]; %South-West
regions(length(regions)+1).name = 'NE';  regions(length(regions)).lat = [37 51]; regions(length(regions)).lon = [-102 -55]; %North-East
regions(length(regions)+1).name = 'SE';  regions(length(regions)).lat = [23 37]; regions(length(regions)).lon = [-100 -75]; %South-East
regions(length(regions)+1).name = 'MA-C';  regions(length(regions)).lat = [10 23]; regions(length(regions)).lon = [-107 -57]; %Meso-america-caribban


% Assuming you have lat/lon grids: lat_grid and lon_grid (same size as the spatial dimensions of your data)
% Make sure lat_grid and lon_grid exist; if not, create them based on your data

for i_season = 1:length(ind_season)
    i_mth_txt = i_mth_txt_loop{i_season};
    ind_season_tmp = ind_season{i_season};
    
    % Take seasonal means (over time dim=3)
    dsVal_tmp       = squeeze(mean(dsValue_4d(:,:,ind_season_tmp,:),3,'omitnan'));
    obsVal_test_tmp = squeeze(mean(obsValue_4d(:,:,ind_season_tmp,:),3,'omitnan'));
    ESM_tmp         = squeeze(mean(ESM_4d(:,:,ind_season_tmp,:),3,'omitnan'));
    dsPIinf_tmp     = squeeze(mean(dsPIValue_5d(:,:,ind_season_tmp,:,1),3,'omitnan'));
    dsPIsup_tmp     = squeeze(mean(dsPIValue_5d(:,:,ind_season_tmp,:,2),3,'omitnan'));
    
    % Loop over regions
    for i_region = 1:length(regions)
        reg = regions(i_region);

        % Create mask of cells inside the region
        region_mask = latgrid >= reg.lat(1) & latgrid <= reg.lat(2) & ...
                      longrid >= reg.lon(1) & longrid <= reg.lon(2);
        
        % Reshape data to [space x time]
        dsVal_reg       = reshape(dsVal_tmp,       [], size(dsVal_tmp, 3));
        obsVal_test_reg = reshape(obsVal_test_tmp, [], size(obsVal_test_tmp, 3));
        ESM_reg         = reshape(ESM_tmp,         [], size(ESM_tmp, 3));
        dsPIinf_reg     = reshape(dsPIinf_tmp,     [], size(dsPIinf_tmp, 3));
        dsPIsup_reg     = reshape(dsPIsup_tmp,     [], size(dsPIsup_tmp, 3));
        
        % Apply region mask: keep only rows (grid cells) inside the region
        ind_in_region = find(region_mask(:));
        dsVal_reg       = dsVal_reg(ind_in_region, :);
        obsVal_test_reg = obsVal_test_reg(ind_in_region, :);
        ESM_reg         = ESM_reg(ind_in_region, :);
        dsPIinf_reg     = dsPIinf_reg(ind_in_region, :);
        dsPIsup_reg     = dsPIsup_reg(ind_in_region, :);
        
        % Optional: add region name to output filename
        suffix_region = [suffix '_' reg.name];
        
        % Call plotting function with region data
        plot_downscaled_timeline(name_var, unit_var, i_mth_txt, obsVal_test_reg, ESM_reg, ...
            dsVal_reg, dsPIinf_reg, dsPIsup_reg, path_fig, suffix_region, name_model, res_plot)
    end
end

clear dsPIValue_5d *_tmp *_reg
close all
disp ('timeline')
...
for i_season = 1:length(ind_season)
    ...
    plot_bias_testing_two_plot(dsVal_tmp, obsVal_test_tmp, ESM_tmp, ...
...
for i_season = 1:length(ind_season)
    ...
    plot_mean_bias_extremes(dsVal_tmp, ESM_tmp, ...
        obsVal_test_tmp, ...
