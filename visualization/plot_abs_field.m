function plot_abs_field(dsValue_map, time_bound, year_start, flag_land, ...
    ...
    if ndims(dsValue_map)==4
        dsValue_map = reshape(dsValue_map,[],size(dsValue_map,3)*size(dsValue_map,4));
    end
    ...
    c_lim = abs_limit;   % or abs_limit depending on function
    cstep = abs_step;    % or abs_step depending on function
