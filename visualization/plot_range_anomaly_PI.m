function plot_range_anomaly_PI(PI_range_map, time_bound, year_start, flag_land, ...
    lat, lon, range_limit, range_step, palette_range, path_shp_file, res_fig, ...
    name_var, i_mth_txt, unit_var, lim_lat, lim_lon, n_color_range, path_fig, suffix)

    % reshape maps to space x time if needed
    if ndims(PI_range_map)==3
        PI_range_map = reshape(PI_range_map,[],size(PI_range_map,3));
    end
    if ndims(PI_range_map)==4
        PI_range_map = reshape(PI_range_map,[],size(PI_range_map,3)*size(PI_range_map,4));
    end
    ...
    for i_panel = 1:size(time_bound,1)
        ...
        plot_map = mean(PI_range_map(:, (time_bound(i_panel_mod,1) - year_start + 1):(time_bound(i_panel_mod,2) - year_start  + 1)), 2, 'omitnan');
        ...
        geosurfm_sub(plot_map, flag_land & not(isnan(plot_map)), ...
            ...
            [], [], title_text, ...
            lim_lat, lim_lon, ...
            range_step, palette_range(n_color_range), path_shp_file, res_fig);
    ...
    sgtitle([i_mth_txt ' differences in mean PCR-downscaled 95% PI range for ' name_var], 'FontSize', 18, 'FontWeight', 'bold');
    ...
    c_lim = range_limit;   % or abs_limit depending on function
    cstep = range_step;    % or abs_step depending on function
    ...
    name_figuresave = fullfile(path_fig, [name_var '_PI range anomaly_' i_mth_txt suffix]);
