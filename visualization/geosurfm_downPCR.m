function geosurfm_downPCR(matrix_map, filter,...
    lat, lon, c_lim, ...
    save_name, clabel, title_text,...
    latlim_map, lonlim_map, ...
    cstep, palette, shp_file_path,res,mean_text,mean_text_suffix)
...
% res = resolution for saving the figure
% mean_text = the text to show for reporting the mean value in the figure.
%   Have it empty (i.e. =[]) if you don't want to show any text.
...
title(title_text)
% fix view options
setm(gca,'fontsize',7,'fontweight','bold','glinewidth',1);
...
if not(isempty(mean_text))
    mean_value = nanmean(matrix_map(:));
    dim = [0.48 0.085  0.25 0.1]; % position of the annotation box
    str = [mean_text ' ', num2str(mean_value, '%.2f') ' ' mean_text_suffix];
